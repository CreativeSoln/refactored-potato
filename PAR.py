    ...
    def _parse_layer(self, layer_el: ET.Element, layerType: str) -> OdxLayer:
        ...
        def _looks_like_belongs(resp_short: str, svc_short: str) -> bool:
            if not resp_short or not svc_short:
                return False
            rs = resp_short.strip().lower()
            ss = svc_short.strip().lower()
...
# Layer metadata
    ...
    def parse_container(self, root: ET.Element) -> OdxContainer:
        ...
        logger.info("[ODXParser] Found layers: PROTOCOL={len(protos)}, "
    ...
    def merge_containers(self, containers: List[OdxContainer]) -> OdxDatabase:
        """
        Merge multiple containers into a flattened OdxDatabase with inter-layer inheritance resolved.
        """
        db = OdxDatabase()
        for c in containers:
        ...
        # Build id_map for link resolution
        id_map: Dict[str, OdxLayer] = {lay.id: lay for lay in all_layers if lay.id}

        # Two-pass link resolution before flattening
        for _ in range(2):
            visited: Set[str] = set()
            for lay in all_layers:
                self._resolve_links_for_layer(lay, id_map, visited)
