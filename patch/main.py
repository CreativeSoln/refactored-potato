...
class MainWindow(QMainWindow):
    ...
    def build_ui(self) -> None:
        ...
        self.cmb_semantic = QComboBox(); self.cmb_semantic.setMinimumWidth(180); self.cmb_semantic.addItem("All semantics", userData=""); self.cmb_semantic.currentIndexChanged.connect(lambda _: self.apply_all_filters(False))
        tb.addWidget(QLabel("Service Semantic:")); tb.addWidget(self.cmb_semantic)

        self.cmb_sid = QComboBox(); self.cmb_sid.setMinimumWidth(180); self.cmb_sid.addItem("All SIDs", userData=None); self.cmb_sid.currentIndexChanged.connect(lambda _: self.apply_all_filters(False))
        ...
        self.tree = QTreeWidget(); self.tree.setHeaderLabels(["Variants & Services", "Type / Semantic", "Info / Value"])
    ...
    def _merge_containers(self, containers: List[Any]) -> OdxDatabase:
        # Accept tuple (name, container) or container
        normalized = [c[1] if isinstance(c, (tuple, list)) and len(c) >= 2 else c for c in containers]
        # Build a database: flatten layers & DOPs
        db = OdxDatabase()
        for cont in normalized:
            db.baseVariants.extend(getattr(cont, 'baseVariants', []) or [])
            db.ecuVariants.extend(getattr(cont, 'ecuVariants', []) or [])
            for layer in (cont.baseVariants + cont.ecuVariants):
                for d in getattr(layer, 'dataObjectProps', []) or []:
                    if d.id:
                        db.allDops[d.id] = d
                for svc in getattr(layer, 'services', []) or []:
                    for msg in ([svc.request] if svc.request else []) + getattr(svc, 'posResponses', []) + getattr(svc, 'negResponses', []):
                        if not msg: continue
    ...
    # Tree population / building (original UI layout preserved)
    def populate_tree(self, initial_build: bool = False) -> None:
        ...
        if initial_build:
            ...
            self.cmb_variant.clear(); self.cmb_variant.addItem("All Variants", userData="")
            for v in variants:
            ...
            self.cmb_semantic.clear(); self.cmb_semantic.addItem("All semantics", userData="")
            semantics: Set[str] = set()
            for v in variants:
                for s in getattr(v, "services", []) or []:
                    sem = getattr(s, "semantic", "");
                    if sem: semantics.add(sem)
            ...
            self.cmb_sid.clear(); self.cmb_sid.addItem("All SIDs", userData=None)
            sids: Set[int] = set()
            for v in variants:
                for s in getattr(v, "services", []) or []:
                    sid = getattr(s, "sid", None)
                    if isinstance(sid, int): sids.add(sid)
        ...
        def m_type_label(kind: str) -> str:
            return {"Request":"REQUEST","Positive Response":"POS_RESPONSE","Negative Response":"NEG_RESPONSE"}.get(kind, kind.upper())

        def cell_value(p: OdxParam) -> str:
            return getattr(p, "value", "") or getattr(p, "displayHex", "") or getattr(p, "displayValue", "") or ""
        ...
        def add_param_recursive(parent_item: QTreeWidgetItem, p: OdxParam) -> bool:
            ...
            if self._filter_text:
                match_self = any(self._filter_text in (t or "").lower() for t in (pname, semantic, third))
                if not match_self:
                    any_child = False
        ...
        for v in variants:
            ...
            for s in getattr(v, "services", []) or []:
                ...
                service_texts = [getattr(s, "shortName", ""), getattr(s, "semantic", ""), getattr(s, "longName", ""), getattr(s, "description", ""), did_text]
                service_passes_text = (not self._filter_text) or any(self._filter_text in (t or "").lower() for t in service_texts)

                svc_title = f"{getattr(s, 'shortName', '')} ({did_text})" if did_text else getattr(s, 'shortName', '')
                s_item = QTreeWidgetItem([svc_title, getattr(s, "semantic", ""), getattr(s, "description", "") or ""])
                ...
                if req: messages.append(("Request", req))
                ...
                for kind, msg in messages:
                    m_item = QTreeWidgetItem([getattr(msg, "shortName", "") or m_type_label(kind), m_type_label(kind), getattr(msg, "longName", "") or ""])
    ...
    def clear_selection(self):
        ...
        self.lbl_selected.setText("0/0 selected")

    def tree_selected(self):
        item = self.tree.currentItem(); self._clear_form(self.details)
        if item:
            obj = item.data(0, Qt.ItemDataRole.UserRole)
            if isinstance(obj, OdxParam): self.show_param_details(obj)
            elif isinstance(obj, OdxService): self.show_service_details(obj)
        self.selectedParams.clear()
        for i in range(self.tree.topLevelItemCount()): self._collect_selected_params(self.tree.topLevelItem(i))
        self.lbl_selected.setText(f"{len(self.selectedParams)}/? selected")

    def tree_item_changed(self, item: QTreeWidgetItem, col: int):
        if self._suppress_item_changed or col != 0 or item is None: return
        state = item.checkState(0)
        self._suppress_item_changed = True
        try: self._set_descendant_checkstate(item, state)
        finally: self._suppress_item_changed = False
        self._refresh_ancestor_state(item)
        self.tree_selected()

    def _set_descendant_checkstate(self, item: QTreeWidgetItem, state: Qt.CheckState) -> None:
        item.setCheckState(0, state)
        for i in range(item.childCount()): self._set_descendant_checkstate(item.child(i), state)

    def _refresh_ancestor_state(self, item: QTreeWidgetItem) -> None:
        parent = item.parent(); if not parent: return
        checked = 0; unchecked = 0
        for i in range(parent.childCount()):
            ch = parent.child(i); st = ch.checkState(0)
            if st == Qt.CheckState.Checked: checked += 1
            elif st == Qt.CheckState.Unchecked: unchecked += 1
            else: checked += 1; unchecked += 1
        if checked and unchecked: parent.setCheckState(0, Qt.CheckState.PartiallyChecked)
        elif checked and not unchecked: parent.setCheckState(0, Qt.CheckState.Checked)
        else: parent.setCheckState(0, Qt.CheckState.Unchecked)
        self._refresh_ancestor_state(parent)

    def _collect_selected_params(self, item: QTreeWidgetItem):
        if item.checkState(0) == Qt.CheckState.Checked:
            p = item.data(0, Qt.ItemDataRole.UserRole)
            if isinstance(p, OdxParam): self.selectedParams.append(p)
        for i in range(item.childCount()): self._collect_selected_params(item.child(i))
    ...
    def _add_detail(self, key: str, value: Any) -> None:
        k = QLabel(str(key)); v = QLabel("" if value is None else str(value)); v.setWordWrap(True); self.details.addRow(k, v)
    ...
    def show_param_details(self, p: OdxParam) -> None:
        ...
        if attrs:
            try: self._add_detail("Attributes", json.dumps(attrs, indent=2))
            except Exception: self._add_detail("Attributes", str(attrs))
    ...
    def show_service_details(self, s: OdxService) -> None:
        ...
        sid = getattr(s, "sid", None)
        if isinstance(sid, int): self._add_detail("SID", f"0x{sid:02X}")
    ...
    def copy_current_did(self) -> None:
        ...
        if not item: self.sb.showMessage("Select a service to copy DID"); return
        obj = item.data(0, Qt.ItemDataRole.UserRole)
        ...
        if service is None:
            ...
            while parent and service is None:
                ...
                if isinstance(pobj, OdxService): service = pobj; break
                parent = parent.parent()
        if not service: self.sb.showMessage("No service selected for DID"); return
        did_hex = getattr(service, "requestDidHex", "") or ""
        if not did_hex: self.sb.showMessage("Selected service has no requestDidHex"); return
